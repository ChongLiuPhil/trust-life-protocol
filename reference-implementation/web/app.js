...
try {
  ...
  const canonical = new URL(location.href); canonical.search=''; canonical.searchParams.set('subject',v.subject.id);
  app.innerHTML = `
    ...
    <div class="grid"><div><div class="eyebrow">Responsible organization</div><p>${esc(v.owner?.name)}</p></div><div><div class="eyebrow">Bundle</div><p class="mono">v${esc(v.bundleVersion)}</p></div><div><div class="eyebrow">Unresolved incidents</div><p class="ok">${esc(v.unresolvedIncidents)}</p></div></div>
    <hr><h2>Claims, evidence and verification</h2>
    ${v.claims.map((claim,ci)=>`<div class="claim"><p><strong>${esc(claim.level)} · ${esc(claim.statement)}</strong></p><p class="mono">${esc(claim.requirementId)}</p>${claim.evidence.map((e,ei)=>`<div class="evidence"><p><strong>Evidence:</strong> ${esc(e.category)} · ${esc(e.capturedAt)}</p><p class="mono">SHA-256 ${esc(e.integrity.digest)}</p><p><span class="digest" data-digest="${ci}-${ei}">checking bytes…</span> · <a href="${esc(e.publicUri)}">open evidence</a></p></div>`).join('')}${claim.verifications.map(x=>`<p><strong>Verification:</strong> ${esc(x.status)} · ${esc(x.method)} · ${esc(x.verifierName||x.verifierOrganizationId)}${x.credentialPublicUri?` · <a href="${esc(x.credentialPublicUri)}">credential payload</a>`:''}</p>`).join('')}${(claim.limitations||[]).map(x=>`<p class="muted">Limitation: ${esc(x)}</p>`).join('')}</div>`).join('')}
    ${c?.limitations?.length?`<hr>${c.limitations.map(x=>`<p class="muted">Conformance limitation: ${esc(x)}</p>`).join('')}`:''}
    <hr><div class="eyebrow">QR-target verification URL</div><p class="mono">${esc(canonical.href)}</p><button id="copy">Copy verification URL</button>
  `;
  document.querySelector('#copy').onclick=async()=>{await navigator.clipboard.writeText(canonical.href);document.querySelector('#copy').textContent='Copied';};
  v.claims.forEach((claim,ci)=>claim.evidence.forEach(async(e,ei)=>{
    const el=document.querySelector(`[data-digest="${ci}-${ei}"]`);
    try{const actual=await sha256(e.publicUri);el.textContent=actual.toLowerCase()===e.integrity.digest.toLowerCase()?'bytes match recorded digest':'DIGEST MISMATCH';}catch(err){el.textContent=`digest check unavailable: ${err.message}`;}
  }));
} catch (e) {
  app.innerHTML=`<h2>Unable to load verification data</h2><p>${esc(e.message)}</p><p class="muted">Use the reference server with <code>npm run serve</code>, or serve the repository over HTTP for static fallback mode.</p>`;
}
